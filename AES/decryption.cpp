...
void shiftrows()
{
	unsigned int temp1 = state[1][3], temp2, temp3;
	for(int i = 3; i > 0; i--)
...
int main(int argc, char const *argv[])
{
	...
	for(int i = 0; i < 4; i++)
		for(int j = 0; j < 4; j++)
			scanf("%x",&key[j][i]);
	cout<<"Enter one block (128 bits) ciphertext as 16 bytes in hexadecimal form\n";
	for(int i = 0; i < 4; i++)
		for(int j = 0; j < 4; j++)
			scanf("%x",&state[j][i]);
	cout<<"Round keys\n";
	...
	cout<<"\n\n";
	decrypt();
	cout<<"\n\nPlain text block as 16 bytes in hexadecimal: ";
	for(int i = 0; i < 4; i++)
		for(int j = 0; j < 4; j++)
			printf("%x ", state[j][i]);

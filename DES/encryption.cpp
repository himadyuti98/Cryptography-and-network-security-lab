...
long key_shift(unsigned long long key, int round_number)
{
	...
	right &= ((one<<28)-1);
	left &= ((one<<28)-1);
	printf("key left: %lx\tkey right: %lx\n", left, right);
	left = (left << 28);
	return (left | right);
}
...
unsigned long round(long right, long key)
{
	...
	for(int i = 0; i < 8; i++)
	{
		...
		int cipherblock = s[i][row][column];
		ciphertext = ciphertext<<4;
		ciphertext |= cipherblock;
	}
	printf("after s boxes: %lx\n", ciphertext);
	return permute(ciphertext, 32, 32, p);
}
...
unsigned long long encrypt(unsigned long long int plainblock, unsigned long long key)
{
	key = parity_drop(key);
	plainblock = permute(plainblock,64, 64, ip_table);
	printf("after initial permutation : %llx\n", plainblock);
	unsigned long one = 1;
	unsigned long right = (((one<<32)-1) & (plainblock));
	unsigned long left = ((one<<32)-1) & (plainblock>>32);
	printf("left: %lx\t right: %lx\n", left, right);
	...
	unsigned long cipherblock = ((right<<32)|left);
	return permute(cipherblock, 64, 64, fp_table);
}
...
int main()
{
	...
	scanf("%llx", &key);
	cout<<"How many blocks of plaintext?"<<endl;
	cin>>number_of_blocks;
	unsigned long long plaintext[number_of_blocks];
	cout<<"Enter plain text in hexadecimal form in blocks of 64 bits"<<endl;
	for(int i = 0; i < number_of_blocks; i++)
		scanf("%llx", &plaintext[i]);
